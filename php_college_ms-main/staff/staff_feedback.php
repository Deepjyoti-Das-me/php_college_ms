...
if (isset($_POST["submit_feedback"])) {
    ...
    if (empty($feedback_type) || empty($subject) || empty($message) || $rating < 1) {
        ...
        $sql = "INSERT INTO staff_feedback (sfid, feedback_type, subject, message, rating, status) 
                VALUES (?, ?, ?, ?, ?, 'pending')";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "isssi", $sfid, $feedback_type, $subject, $message, $rating);
...
$feedback_query = "SELECT * FROM staff_feedback WHERE sfid = $sfid ORDER BY created_at DESC";
...
<head>
    ...
    <title>Give Feedback - Staff Panel</title>
