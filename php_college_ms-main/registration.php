    ...
    if (count($errors) == 0) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $uniqueId = $prefix . str_pad($userId, $idLength, '0', STR_PAD_LEFT);
                
                // Insert into respective table
                if ($userType == 'student') {
                    $sql2 = "INSERT INTO students (user_id, student_id, enrollment_date) VALUES (?, ?, CURDATE())";
                    $stmt2 = mysqli_prepare($conn, $sql2);
                    if ($stmt2) {
                        mysqli_stmt_bind_param($stmt2, "is", $userId, $uniqueId);
                        mysqli_stmt_execute($stmt2);
                        mysqli_stmt_close($stmt2);
                    }
                } elseif ($userType == 'teacher') {
                    $sql2 = "INSERT INTO teachers (user_id, teacher_id, hire_date) VALUES (?, ?, CURDATE())";
                    $stmt2 = mysqli_prepare($conn, $sql2);
                    if ($stmt2) {
                        mysqli_stmt_bind_param($stmt2, "is", $userId, $uniqueId);
                        mysqli_stmt_execute($stmt2);
                    ...
                    $sql2 = "INSERT INTO staff (user_id, staff_id, hire_date) VALUES (?, ?, CURDATE())";
                    $stmt2 = mysqli_prepare($conn, $sql2);
                    if ($stmt2) {
                        mysqli_stmt_bind_param($stmt2, "is", $userId, $uniqueId);
                        mysqli_stmt_execute($stmt2);
                ...
                // Regenerate CAPTCHA for next registration
                $_SESSION['captcha_num1'] = rand(5, 15);
                $_SESSION['captcha_num2'] = rand(5, 15);
                
                echo "<div class='alert alert-success'>Registration successful! Your " . ucfirst($userType) . " ID is: <strong>$uniqueId</strong></div>";
