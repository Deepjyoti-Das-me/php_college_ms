...
if (isset($_POST["request_leave"])) {
    ...
    if (empty($leave_type) || empty($start_date) || empty($end_date) || empty($reason)) {
        ...
        $sql = "INSERT INTO student_leave (sid, leave_type, start_date, end_date, days, reason, status) 
                VALUES (?, ?, ?, ?, ?, ?, 'pending')";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "isssis", $sid, $leave_type, $start_date, $end_date, $days, $reason);
...
// Get leave history
$leave_query = "SELECT * FROM student_leave WHERE sid = $sid ORDER BY created_at DESC";
...
<head>
    ...
    <title>Leave Request - Student Panel</title>
