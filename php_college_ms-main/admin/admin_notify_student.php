...
if (isset($_POST["send_notification"])) {
    ...
    if (empty($title) || empty($message)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            $success = "Notification sent successfully to all students!";
            $_POST = array();
        } else {
...
// Get all students for individual notification
$students_query = "SELECT s.sid, s.roll_no, s.fname, s.lname, u.email 
                   FROM students s 
...
<head>
    ...
    <title>Notify Students - Admin Panel</title>
...
<body>
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-bell"></i> Notify Students</h1>
        </div>
                        ...
                        <div class="form-group" style="grid-column: 1 / -1;">
                            ...
                            <select name="target_id">
                                ...
                                <?php while ($student = mysqli_fetch_assoc($students_result)): ?>
                                    <option value="<?php echo $student['sid']; ?>" <?php echo (isset($_POST['target_id']) && $_POST['target_id'] == $student['sid']) ? 'selected' : ''; ?>>
                                        <?php echo $student['roll_no'] . ' - ' . $student['fname'] . ' ' . $student['lname']; ?>
                                    </option>
                                <?php endwhile; ?>
                            </select>
                            <small>Select "All Students" to send to everyone, or choose a specific student</small>
                        </div>
                    </div>
                </div>
