...
if (isset($_POST["update_leave"]) && isset($_POST["leave_id"])) {
    ...
    $update_query = "UPDATE student_leave SET status = '$status', approved_by = $approved_by, remarks = '$remarks' WHERE id = $leave_id";
...
$leave_query = "SELECT sl.*, s.roll_no, s.fname, s.lname, c.course_code, c.cname
                FROM student_leave sl
                JOIN students s ON sl.sid = s.sid
...
<head>
    ...
    <title>Student Leave - Admin Panel</title>
...
<body>
    ...
    <main class="main-content">
        <div class="page-header">
            <h1><i class="fas fa-calendar-minus"></i> Student Leave Management</h1>
        </div>
                ...
                <tbody>
                    <?php if (mysqli_num_rows($leave_result) > 0): ?>
                        <?php while ($leave = mysqli_fetch_assoc($leave_result)): ?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($leave['roll_no']); ?></strong></td>
                                <td><?php echo htmlspecialchars($leave['fname'] . ' ' . $leave['lname']); ?></td>
                                <td><?php echo htmlspecialchars($leave['course_code'] . ' - ' . $leave['cname']); ?></td>
